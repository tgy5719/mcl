...
class ZDelivSlip(models.Model):
    ...
    zstock_total_amount = fields.Float(string="total amount", compute='_calculate_price_total', store=True)

    @api.depends('move_ids_without_package','move_ids_without_package.z_price_unit')
    def _calculate_price_total(self):
        for l in self:
            l.zstock_total_amount = sum(line.z_price_unit*line.qty_done for line in l.move_line_ids_without_package)
    @api.multi
    @api.depends('amount_total_number','zstock_total_amount','tall')
    def _cats(self):
        for line in self:
            cate = str(line.zstock_total_amount)
            line.z_price_total_number = str(cate)
            line.zstock_total_amount = round(line.zstock_total_amount)
            line.tall = round(line.zstock_total_amount)
